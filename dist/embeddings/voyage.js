 ...
 */
export async function embed(texts, inputType, model) {
        ...
        try {
            ...
            if (!response.ok) {
                if (!isRetryableStatus(response.status)) {
                    ...
                    throw new NonRetryableVoyageError(`Voyage embeddings request failed with non-retryable status ${response.status}`);
                }
    ...
    for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
        try {
            ...
            if (!response.ok) {
                if (!isRetryableStatus(response.status)) {
                    // Non-retryable (e.g. 400 bad request): fail immediately, no retry.
                    throw new NonRetryableVoyageError(`Voyage rerank request failed with non-retryable status ${response.status}`);
                }
