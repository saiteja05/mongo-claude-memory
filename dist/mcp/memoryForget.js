 ...
 */
export async function runMemoryForget(db, params, deps = { compileBrief: defaultCompileBrief }) {
    const beliefs = db.collection(BELIEFS);
    const filter = { _id: toFilterId(params.beliefId), project: params.project };
    ...
    if (matched) {
        ...
        try {
            ...
            // enough to diagnose.
            console.error(`memory_forget: brief recompile failed after tombstone (${err instanceof Error ? err.name : "unknown error"}); the tombstoned belief drops out at the next consolidation recompile`);
        }
    }
