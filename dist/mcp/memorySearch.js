...
async function defaultAggregate(db, pipeline) {
    return db.collection(BELIEFS).aggregate(pipeline).toArray();
}
async function defaultUpdateUseCount(db, ids) {
    if (ids.length === 0)
        return;
    await db.collection(BELIEFS).updateMany({ _id: { $in: ids } }, { $inc: { use_count: 1 }, $set: { last_used: new Date() } });
}
const defaultDeps = {
    embed: voyageEmbed,
    rerank: voyageRerank,
 ...
 */
function isRerankUnsupportedError(err) {
    const message = errMsg(err).toLowerCase();
    return (message.includes("$rerank") ||
 ...
 */
function nativeRerankKnownUnavailable() {
    if (rerankCache.available !== false)
        return false;
    if (rerankCache.negativeCachedAt !== null &&
 ...
 */
export async function computeQueryEmbedding(query, embedFn) {
    try {
        const vectors = await embedFn([query], "query");
        return vectors[0] ?? null;
    }
    catch (err) {
        console.error(`[memorySearch] query embedding failed, falling back to text-only: ${errMsg(err)}`);
        return null;
    }
}
function statusProjectFilter(project, scope) {
    const filter = { project, status: "active" };
    if (scope)
        filter.scope = scope;
    return filter;
}
function searchCompoundFilter(project, scope) {
    const clauses = [
        { equals: { path: "status", value: "active" } },
        { equals: { path: "project", value: project } },
    ];
    if (scope)
        clauses.push({ equals: { path: "scope", value: scope } });
    return clauses;
}
/**
 ...
 */
export function buildFullPipeline(query, queryVector, project, scope) {
    return [
        {
            $rankFusion: {
                input: {
...
export function buildTextOnlyPipeline(query, project, scope) {
    return [
        {
            $search: {
                index: TEXT_INDEX,
...
export function buildVectorOnlyPipeline(queryVector, project, scope) {
    return [
        {
            $vectorSearch: {
                index: VECTOR_INDEX,
 ...
 */
export function buildFullPipelineAuto(query, project, model, scope) {
    return [
        {
            $rankFusion: {
                input: {
...
export function buildVectorOnlyPipelineAuto(query, project, model, scope) {
    return [
        {
            $vectorSearch: {
                index: VECTOR_INDEX_AUTO,
 ...
 */
async function tryVoyageRerank(baseResults, query, limit, rerankFn) {
    ...
    try {
        ...
        console.error(`[memorySearch] Voyage rerank fallback failed, returning fused results unranked: ${errMsg(err)}`);
        return { docs: baseResults.slice(0, limit), reranked: false };
    }
}
/**
 ...
 */
async function runBasePipeline(db, query, queryVector, project, scope, aggregate) {
    if (queryVector) {
        ...
        try {
            ...
            console.error(`[memorySearch] full $rankFusion pipeline failed, falling back to vector-only: ${errMsg(err)}`);
 ...
 */
async function runBasePipelineAuto(db, query, project, scope, model, aggregate) {
    ...
    try {
        ...
        console.error(`[memorySearch] full autoEmbed $rankFusion pipeline failed, falling back to vector-only: ${errMsg(err)}`);
...
function toResultItem(doc, scoreField) {
 ...
 */
export async function runMemorySearch(db, params, deps = {}) {
    ...
    const base = config.embeddingMode === "auto"
        ? await runBasePipelineAuto(db, params.query, params.project, params.scope, config.voyageModel, aggregate)
        : await runBasePipeline(db, params.query, await computeQueryEmbedding(params.query, embedFn), params.project, params.scope, aggregate);
    if (!base) {
    ...
    if (eligibleForNativeRerank) {
        ...
        if (config.rerankMode === "appside") {
            ...
            try {
                ...
                console.error(`[memorySearch] native $rerank unavailable (rerankMode=native, no Voyage fallback): ${errMsg(err)}`);
            ...
            try {
                ...
                if (isRerankUnsupportedError(err)) {
                    console.error(`[memorySearch] native $rerank unsupported/disabled, caching negative result for 10 minutes and falling back to Voyage rerank: ${errMsg(err)}`);
                    rerankCache = { available: false, negativeCachedAt: rerankCacheNow() };
                }
                else {
                    console.error(`[memorySearch] native $rerank failed transiently, falling back to Voyage rerank for this call only (not cached): ${errMsg(err)}`);
                }
                const fallback = await tryVoyageRerank(baseResults, params.query, limit, rerankFn);
                finalDocs = fallback.docs;
                scoreField = fallback.reranked ? "voyageRerankScore" : "fusionScore";
    ...
    // runMemorySearch.
    void updateUseCount(db, finalDocs.map((doc) => doc._id)).catch((err) => {
