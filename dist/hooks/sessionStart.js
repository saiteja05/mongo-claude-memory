...
import { writeBriefCache, readBriefCache } from "../briefs/briefCache.js";
...
export const TRUST_PREAMBLE = "[memory notice: the lines below are project facts recalled from this project's own " +
...
export async function buildAdditionalContext(input, deps) {
    ...
    const cachedParts = [];
    if (cached.global)
        cachedParts.push(cached.global);
    if (cached.project)
        cachedParts.push(cached.project);
    const notice = "[memory notice: this brief was served from a local cache because the live memory " +
