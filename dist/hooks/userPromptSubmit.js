 ...
 */
export function shouldCaptureAsHashLine(promptText) {
 ...
 */
export async function captureUserPromptSubmit(input, deps) {
    try {
        const promptText = resolvePromptText(input);
        if (!shouldCaptureAsHashLine(promptText))
            return;
 ...
 */
export async function runUserPromptSubmitHook(input, deps) {
    const promptText = resolvePromptText(input);
    if (!shouldCaptureAsHashLine(promptText))
        return { outcome: "skipped", pendingWrite: null };
    try {
        const write = (async () => {
