        ...
        try {
            ...
            if (!response.ok) {
                if (!isRetryableStatus(response.status)) {
                    throw new NonRetryableLLMError(`Anthropic messages request failed with non-retryable status ${response.status}`);
                }
                throw new Error(`Anthropic messages request failed with status ${response.status}`);
            }
            ...
            if (json.stop_reason === "max_tokens") {
                throw new NonRetryableLLMError("extraction output truncated by max_tokens; reduce batch size");
            }
            const toolUse = json.content.find((block) => block.type === "tool_use" && block.name === toolName);
            if (!toolUse) {
                throw new NonRetryableLLMError(`Anthropic response did not include a tool_use block for "${toolName}"`);
            }
