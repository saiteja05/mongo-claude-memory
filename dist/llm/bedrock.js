...
export async function callWithTool(systemPrompt, userPrompt, toolName, toolSchema) {
    ...
    for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
        ...
        try {
            ...
            const toolUse = blocks.find((block) => block.toolUse !== undefined && block.toolUse.name === toolName);
            if (!toolUse || !toolUse.toolUse) {
                throw new NonRetryableLLMError(`Bedrock response did not include a toolUse block for "${toolName}"`);
            }
