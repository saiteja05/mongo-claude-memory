        ...
        try {
            ...
            if (!response.ok) {
                if (!isRetryableStatus(response.status)) {
                    throw new NonRetryableLLMError(`Ollama chat request failed with non-retryable status ${response.status}`);
                }
            ...
            const toolCall = json.message.tool_calls?.find((call) => call.function.name === toolName);
            if (!toolCall) {
                throw new NonRetryableLLMError(`Ollama response did not include a tool call for "${toolName}"`);
            }
