 ...
 */
export async function acquireLease(db, project, runId, leaseMs) {
    const now = new Date();
    try {
        await db.collection(LOCKS).findOneAndUpdate({ _id: `consolidate:${project}`, heldUntil: { $lt: now } }, { $set: { holder: runId, heldUntil: new Date(now.getTime() + leaseMs) } }, { upsert: true, returnDocument: "after" });
        return true;
    }
    catch (err) {
 ...
 * holder's lease.
 */
export async function releaseLease(db, project, runId) {
    await db.collection(LOCKS).updateOne({ _id: `consolidate:${project}`, holder: runId }, { $set: { heldUntil: new Date(0) } });
}
/**
 ...
 * stop).
 */
export async function renewLease(db, project, runId, leaseMs) {
    const now = new Date();
    const result = await db.collection(LOCKS).updateOne({ _id: `consolidate:${project}`, holder: runId }, { $set: { heldUntil: new Date(now.getTime() + leaseMs) } });
