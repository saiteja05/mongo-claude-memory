...
function zeroReport(beliefsScanned) {
    return {
        beliefsScanned,
        pairsFound: 0,
        pairsArbitrated: 0,
 ...
 */
export async function runReconcileSweep(db, project, deps) {
    ...
    for (const belief of beliefs) {
        ...
        let near;
        try {
            near = await findSimilarBeliefs(db, project, belief.scope, useTextQuery ? [] : belief.embedding, deps.threshold, RECONCILE_TOP_K, useTextQuery
    ...
    if (report.skippedCap > 0) {
        console.error(`[reconcile] project="${project}": pair cap (${deps.maxPairs}) reached, skipped ${report.skippedCap} additional pair(s)`);
    }
    ...
    for (const [beliefA, beliefB] of pairs) {
        ...
        if (archivedIds.has(idA) || archivedIds.has(idB))
            continue;
        try {
            ...
            if (verdict === "unrelated")
                continue;
            const now = new Date();
            const archiveResult = await beliefsCollection.updateOne({ _id: toFilterId(olderIdStr), status: "active", version: older.version }, { $set: { status: "archived", updated_at: now }, $inc: { version: 1 } });
            ...
            if (verdict === "supersedes") {
                ...
                if (newer.supersedes === null || newer.supersedes === undefined) {
                    await beliefsCollection.updateOne({ _id: toFilterId(newerIdStr), status: "active", supersedes: null }, { $set: { supersedes: olderIdStr, updated_at: now }, $inc: { version: 1 } });
                }
                ...
                await beliefsCollection.updateOne({ _id: toFilterId(newerIdStr), status: "active" }, { $addToSet: { observation_ids: { $each: olderObservationIds } }, $inc: { version: 1 } });
 ...
 */
export function formatReconcileReport(project, report) {
    const lines = [];
    lines.push(`[reconcile] project="${project}": scanned ${report.beliefsScanned} active belief(s), found ${report.pairsFound} pair(s), arbitrated ${report.pairsArbitrated} pair(s)`);
    lines.push(`Archived ${report.archivedSupersedes.length} superseded belief(s) and ${report.archivedDuplicates.length} duplicate belief(s)`);
    ...
    lines.push(`Skipped: ${report.skippedCap} over the pair cap, ${report.skippedContention} due to contention, ` +
        `${report.skippedErrors} due to errors`);
    lines.push(report.recompiledScopes.length > 0
