 ...
 */
export async function runRollback(db, runId, deps = { compileBrief: defaultCompileBrief }) {
    ...
    const revertedBeliefs = [];
    const restoredBeliefs = [];
    const needsManualReview = [];
    const scopesToRecompile = new Set();
    ...
    for (const belief of touchedBeliefs) {
        ...
        const tombstoneResult = await beliefsCollection.updateOne({ _id: belief._id, version: belief.version }, { $set: { status: "tombstoned", updated_at: now }, $inc: { version: 1 } });
        ...
        if (belief.supersedes) {
            ...
            if (newerSuccessor) {
                ...
                const restoreResult = await beliefsCollection.updateOne({ _id: toFilterId(String(belief.supersedes)), status: "archived" }, { $set: { status: "active", updated_at: now }, $inc: { version: 1 } });
    ...
    const resetResult = await observationsCollection.updateMany({ _id: { $in: runObsIds }, status: "consolidated" }, { $set: { status: "pending" }, $unset: { run_id: "", claimed_at: "" } });
    const recompiledScopes = [];
 ...
 */
export function formatRollbackReport(runId, result) {
    const lines = [];
    lines.push(`[rollback] run_id="${runId}": reverted ${result.revertedBeliefs.length} belief(s), ` +
    ...
    if (result.needsManualReview.length > 0) {
        ...
        for (const item of result.needsManualReview) {
            lines.push(`  - belief ${item.beliefId}: this run contributed ${item.runObservationIds.length} of ` +
                `${item.observationIds.length} observation(s)` +
                (item.reason ? ` (${item.reason})` : ""));
        }
    }
    lines.push(result.recompiledScopes.length > 0
