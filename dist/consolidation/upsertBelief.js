 ...
 */
export async function findSimilarBeliefs(db, project, scope, embedding, threshold, k, options = {}) {
    const filter = scope === "core"
        ? { scope: "core", status: "active" }
        : { project, scope, status: "active" };
    const vectorSearchStage = options.mode === "auto"
        ? {
 ...
 */
export async function findSimilarBelief(db, project, scope, embedding, threshold, options = {}) {
...
function newBeliefDoc(project, candidate, embedding, now, supersedes, evidenceAt) {
    const doc = {
        project,
 ...
 */
async function mergeIntoActiveBelief(collection, similar, candidate, evidenceAt, now) {
    ...
    for (let attempt = 1; attempt <= maxAttempts && existing; attempt++) {
        const existingObservationIds = Array.isArray(existing.observation_ids)
            ? existing.observation_ids
            : [];
        const mergedObservationIds = Array.from(new Set([...existingObservationIds, ...candidate.observation_ids]));
        const existingText = typeof existing.text === "string" ? existing.text : similar.text;
        ...
        const existingEvidenceAt = existing.last_evidence_at instanceof Date ? existing.last_evidence_at : new Date(0);
        ...
        const updateResult = await collection.updateOne({ _id: existingFilterId, status: "active", version: existing.version }, {
            $set: setFields,
            $inc: { version: 1 },
        });
 ...
 * taken.
 */
export async function upsertBelief(db, project, candidate, embedding, dedupeSimilarityThreshold, options = {}, candidateEvidenceAt, 
...
reconcileOptions) {
    ...
    if (candidate.supersedes_belief_id) {
        ...
        if (old) {
            await collection.updateOne({ _id: oldFilterId }, 
            ...
            const inserted = await collection.insertOne(newBeliefDoc(project, candidate, embedding, now, candidate.supersedes_belief_id, evidenceAt));
            return { beliefId: String(inserted.insertedId), action: "supersede" };
        }
    }
    const similar = await findSimilarBelief(db, project, candidate.scope, embedding ?? [], dedupeSimilarityThreshold, { mode: options.mode, model: options.model, queryText: candidate.text });
    if (similar) {
    ...
    if (reconcileOptions && reconcileOptions.threshold < 1) {
        try {
            const near = await findSimilarBeliefs(db, project, candidate.scope, embedding ?? [], reconcileOptions.threshold, RECONCILE_TOP_K, { mode: options.mode, model: options.model, queryText: candidate.text });
            ...
            if (near.length > 0) {
                ...
                for (const verdict of ordered) {
                    if (verdict.verdict === "supersedes") {
                        ...
                        const archiveFilter = candidate.scope === "core"
                        ...
                        const inserted = await collection.insertOne(newBeliefDoc(project, candidate, embedding, now, verdict.beliefId, evidenceAt));
                        return { beliefId: String(inserted.insertedId), action: "supersede" };
                    }
    ...
    const inserted = await collection.insertOne(newBeliefDoc(project, candidate, embedding, now, undefined, evidenceAt));
