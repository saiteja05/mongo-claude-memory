 ...
 */
function render(sorted, tokenCap, scopeKey) {
    ...
    if (droppedCount > 0) {
        console.error(`[compileBrief] scope="${scopeKey}": token cap (${tokenCap}) reached, dropped ${droppedCount} ` +
 ...
 */
export async function compileBrief(db, scopeKey) {
    ...
    const existing = await briefsCollection.findOne({ _id: briefId });
    const generation = (existing?.generation ?? 0) + 1;
    await briefsCollection.replaceOne({ _id: briefId }, {
