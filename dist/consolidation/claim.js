 ...
 */
export async function reclaimStale(db, project, reclaimAfterMs, runId) {
    const threshold = new Date(Date.now() - reclaimAfterMs);
    const now = new Date();
    const lock = await db.collection(LOCKS).findOne({ _id: `consolidate:${project}` });
    const liveHolder = lock && lock.heldUntil > now && lock.holder !== runId ? lock.holder : null;
    const filter = { project, status: "claimed", claimed_at: { $lt: threshold } };
    if (liveHolder !== null) {
 ...
 */
export async function claimBatch(db, project, runId, batchSize, maxChars = DEFAULT_BATCH_MAX_CHARS) {
    ...
    const now = new Date();
    await collection.updateMany({ _id: { $in: ids }, status: "pending" }, { $set: { status: "claimed", run_id: runId, claimed_at: now } });
