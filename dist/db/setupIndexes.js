...
async function ensureTtlIndex(db) {
    ...
    await collection.createIndex({ expiresAt: 1 }, { name: "expiresAt_ttl", expireAfterSeconds: 0 });
    console.log(`[indexes] created TTL index "${OBSERVATIONS}.expiresAt_ttl"`);
}
async function ensureDroppedCandidatesTtlIndex(db) {
...
async function ensureBeliefsCompoundIndex(db) {
    ...
    await collection.createIndex({ project: 1, scope: 1, status: 1 }, { name: "project_scope_status" });
    console.log(`[indexes] created compound index "${BELIEFS}.project_scope_status"`);
}
async function ensureBeliefsTtlIndex(db) {
    const collection = db.collection(BELIEFS);
    const existing = await collection.indexes();
...
async function ensureSearchIndex(db, collectionName, name, type, definition) {
    ...
    try {
        ...
        console.log(`[search indexes] could not list search indexes on "${collectionName}" ` +
...
export async function setupIndexes() {
    ...
    console.log(`[indexes] "${BRIEFS}" documents are keyed by _id ("brief:global" or "brief:<project>"); ` +
