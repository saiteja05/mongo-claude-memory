 ...
 */
export async function runRemember(text, cwd, deps) {
    if (text.length === 0) {
    ...
    if (!deps.hasMongoConfig()) {
        return {
            ok: false,
            stderr: "Memory is not configured yet: set MDB_MCP_CONNECTION_STRING or MEMORY_MONGODB_URI.",
        };
    }
    try {
 ...
 */
async function readArgText(args) {
...
async function main() {
    ...
    try {
        ...
        process.stderr.write(`Failed to read remembered text: ${err instanceof Error ? err.name : "unknown error"}\n`);
        process.exit(1);
        return;
    }
    try {
        ...
        process.stderr.write(`Failed to save to memory: ${err instanceof Error ? err.name : "unknown error"}\n`);
        exitCode = 1;
    }
    finally {
