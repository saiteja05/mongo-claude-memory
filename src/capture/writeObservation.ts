...
export interface WriteObservationParams {
  ...
  text: string;
  // Set together, only for a chunk produced by SessionEnd's multi-chunk
  // transcript capture: consecutive slices of one session transcript,
  // 0-based index plus the total chunk count.
  chunk_index?: number;
  chunk_count?: number;
}
 ...
 */
function buildObservationDoc(params: WriteObservationParams, config: Config): Document {
 ...
 */
export async function writeObservationsBulk(
  db: Db,
  paramsList: WriteObservationParams[]
): Promise<unknown> {
  const config = loadConfig();
  const base = new Date();
  ...
  const result = await db.collection<Document>(OBSERVATIONS).insertMany(docs, { ordered: true });
