 ...
 */
function connectWithHardTimeout(client: MongoClient): Promise<MongoClient> {
  return new Promise<MongoClient>((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      reject(
        new Error(
          `MongoDB connect() exceeded the ${CONNECT_ATTEMPT_TIMEOUT_MS}ms hard timeout (this bound covers DNS SRV/TXT resolution for mongodb+srv:// URIs too, not just server selection)`
        )
      );
