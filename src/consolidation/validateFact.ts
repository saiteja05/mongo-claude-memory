...
export interface ValidateFactResult {
  valid: boolean;
  reason?: string;
}
 ...
 */
export function validateBeliefText(text: string): ValidateFactResult {
...
export function validateCandidateFact(candidate: {
  ...
  if (!VALID_SCOPES.has(candidate.scope as string)) {
    return { valid: false, reason: `invalid scope: ${JSON.stringify(candidate.scope)}` };
  }
  if (!VALID_TYPES.has(candidate.type as string)) {
