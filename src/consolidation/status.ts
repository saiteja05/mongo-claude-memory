...
export interface StatusReport {
  observationCounts: Array<{ project: string; status: string; count: number }>;
...
interface GroupedCount {
  _id: { project?: unknown; status?: unknown };
  count: number;
}

function groupByProjectStatus(docs: GroupedCount[]): Array<{ project: string; status: string; count: number }> {
  return docs.map((doc) => ({
...
export async function getStatusReport(db: Db, reclaimAfterMs: number): Promise<StatusReport> {
  ...
  const observationCountsRaw = (await observationsCollection
    ...
    .toArray()) as unknown as GroupedCount[];
  ...
  const lockDocs = await db.collection<Document>(LOCKS).find({}).toArray();
  const locks = lockDocs.map((doc) => ({
    project: lockProjectLabel(doc._id),
    holder: String(doc.holder ?? ""),
    heldUntil: doc.heldUntil as Date,
    live: (doc.heldUntil as Date) > now,
  }));
  ...
  const briefDocs = await db.collection<Document>(BRIEFS).find({}).toArray();
...
export function formatStatusReport(report: StatusReport): string {
  ...
  if (report.locks.length === 0) {
    ...
    for (const lock of report.locks) {
      lines.push(
        `  - project="${lock.project}", holder=${lock.holder}, heldUntil=${lock.heldUntil.toISOString()}, ` +
          `live=${lock.live}`
      );
    }
  }
  ...
  if (report.briefs.length === 0) {
    ...
    for (const brief of report.briefs) {
      lines.push(
        `  - id=${brief.id}, generation=${brief.generation}, beliefCount=${brief.beliefCount}, ` +
