...
import type { BriefResult } from "./fetchBrief.js";
...
export function briefCacheDir(): string {
  return (
    process.env.MEMORY_BRIEF_CACHE_DIR ||
...
function cacheFileFor(projectKey: string): string {
...
export interface StoredBriefCache {
  global: string | null;
  project: string | null;
 ...
 */
export function writeBriefCache(projectKey: string, briefs: BriefResult): void {
  try {
    if (!briefs.global && !briefs.project) return;
    ...
    if (Buffer.byteLength(data, "utf8") > MAX_CACHE_BYTES) return;
 ...
 */
export function readBriefCache(projectKey: string, maxAgeDays: number): StoredBriefCache | null {
  try {
 ...
 */
export function deleteBriefCache(projectKey: string): void {
  try {
