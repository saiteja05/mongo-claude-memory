...
const MODES = ["sessionStart", "userPromptSubmit", "sessionEnd"] as const;
...
export function missingDepsMessage(pluginRoot: string): string {
  return (
    `[Recall memory plugin: dependencies not installed yet. Run \`cd "${pluginRoot}" && ` +
